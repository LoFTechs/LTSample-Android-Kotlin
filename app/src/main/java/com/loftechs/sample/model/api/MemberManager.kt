...
import com.loftechs.sdk.im.extension.rx.inviteMembers
import com.loftechs.sdk.im.extension.rx.kickMembers
import com.loftechs.sdk.im.extension.rx.queryChannelMembersByChID
...
object MemberManager {
     ...
     */
    private fun queryChannelMembersByChID(
        receiverID: String,
        chID: String,
        lastUserID: String
     ...
     */
    fun queryAllChannelMembers(
        receiverID: String,
        chID: String
    ): Observable<ArrayList<MemberEntity>> {
        ...
        return Observable
            ...
            .map { memberResponse: LTMemberPrivilege ->
                MemberEntity(
                    memberResponse.userID, memberResponse.nickname, memberResponse.phoneNumber,
                    memberResponse.affID, memberResponse.roleID, memberResponse.profileID
                )
            }
            .collect(
                { ArrayList() },
                BiConsumer(ArrayList<MemberEntity>::add) as BiConsumer<ArrayList<MemberEntity>, MemberEntity>
            )
            .toObservable()
            .doOnNext(Consumer { channelMemberEntities -> // 若是最後一批，就把lastUserID 壓為 null
                ...
                lastUserID[0] =
                    if (channelMemberEntities.size == BATCH_COUNT) lastMemberEntity.userID else null
            })
    ...
    fun kickMembers(
        ...
        return Observable.fromIterable(kickMembersUserIDs)
            ...
            .collect(
                { HashSet() },
                BiConsumer(HashSet<LTMemberModel>::add) as BiConsumer<HashSet<LTMemberModel>, LTMemberModel>
            )
            .toObservable()
