    ...
    private val token: Observable<String>
        get() {
            ...
            return if (token.isNotEmpty()) {
                getTokenTime(token)
                    .flatMap { time: Long ->
                        if (time == 0L || System.currentTimeMillis() > time - TimeUnit.MINUTES.toMillis(
                                10
                            )
                        ) {
        ...
        return Observable.create { emitter: ObservableEmitter<Long> ->
            ...
            token?.split("\\.".toRegex())?.toTypedArray()?.let {
                if (it.size > 2) {
                    ...
                    val tokenBody =
                        String(Base64.decode(base64EncodedBody, Base64.NO_WRAP), Charsets.UTF_8)
    ...
    fun register(account: String, pwd: String): Observable<RegisterResponse> {
        return token
            .flatMap {
                ...
                val registerRequest =
                    RegisterRequest(BuildConfig.LTSDK_TurnKey, Collections.singletonList(user))
