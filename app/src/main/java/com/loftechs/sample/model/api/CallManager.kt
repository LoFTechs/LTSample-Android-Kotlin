...
object CallManager : LTCallStateListener, LTCallNotificationListener {
    ...
    private var ltCallCenterManager: LTCallCenterManager = LTSDK.getInstance().getCallCenterManager()
    ...
    fun doOutgoingCallWithUserID(receiverID: String, account: String, userID: String): Observable<Boolean> {
        ...
        getCallManager()
                .subscribe(object : Observer<LTCallCenterManager> {
                    ...
                    override fun onNext(centerManager: LTCallCenterManager) {
                        val number = if (!incomingCallMessage.callOptions.semiUID.isNullOrEmpty()) incomingCallMessage.callOptions.semiUID else incomingCallMessage.callOptions.phoneNumber
                        if (ltCall == null) {
                            val builder = setNotificationBuilder(incomingCallMessage.receiver, incomingCallMessage.callOptions.userID, "", number
                                    ?: "", true)
                            centerManager.setAndroidNotification(builder, 1)
                        }
                        val tempIncomingCall = centerManager.startCallWithNotificationMessage(incomingCallMessage, this@CallManager)
                        if (tempIncomingCall != null) {
                            ...
                            startCallActivity(getCallIntent(incomingCallMessage.receiver, incomingCallMessage.callOptions.userID, CallState.IN))
    ...
    private fun getCallIntent(receiverID: String, callUserID: String, callState: CallState): Intent {
    ...
    fun getCallLog(receiverID: String, startTime: Long, count: Int): Observable<ArrayList<CallLogData>> {
        return ltCallCenterManager.queryCDRWithUserID(receiverID, startTime, count)
                ...
                .map {
                    CallLogData(it.senderID, it.callID, it.callStartTime, it.callEndTime,
                            it.calleeInfo, it.callerInfo, it.billingSecond, getCallType(receiverID, it.callerInfo.userID, it.billingSecond.toLong()))
                }
                .collect({ ArrayList() }, BiConsumer(ArrayList<CallLogData>::add) as BiConsumer<ArrayList<CallLogData>, CallLogData>)
                .toObservable()
    }
    ...
    private fun setNotificationBuilder(receiverID: String, callUserID: String, content: String, displayName: String, isIncomingCall: Boolean): NotificationCompat.Builder? {
        var notifyMessage = content
        val appName = SampleApp.context.applicationInfo.loadLabel(SampleApp.context.packageManager).toString()
        ...
        try {
            ...
            val notificationContentIntent = PendingIntent.getActivity(
     ...
     */
    private fun getNotificationBuilder(channelID: String, channelName: String, importance: Int): NotificationCompat.Builder {
    ...
    override fun onLTCallStateConnected(call: LTCall) {
        ...
        val builder = setNotificationBuilder(ltCallCenterManager.mainUserID, call.callOptions.userID, "In Call", "", false)
    ...
    override fun onLTCallCDRNotification(callCDRNotificationMessage: LTCallCDRNotificationMessage) {
        ...
        EventBus.getDefault().post(CallCDREvent(callCDRNotificationMessage.receiver,
                CallLogData(callCDRNotificationMessage.senderID, callCDRNotificationMessage.callID, callCDRNotificationMessage.callStartTime,
                        callCDRNotificationMessage.callEndTime, callCDRNotificationMessage.calleeInfo,
                        callCDRNotificationMessage.callerInfo, callCDRNotificationMessage.billingSecond, getCallType(callCDRNotificationMessage.receiver, callCDRNotificationMessage.callerInfo.userID, callCDRNotificationMessage.billingSecond.toLong()))))
    }
}